...
아래 3개 커맨드를 **순서대로** 실행하라. 각 커맨드가 끝날 때마다 결과를 확인한 뒤 다음 커맨드로 넘어간다.
...
- **모두 통과 (이슈 없음)**: Phase 2로 바로 진행한다.
- **🟡 Warning 또는 🔴 Critical이 하나라도 있는 경우**:
  1. 발견된 이슈를 심각도별로 정리하여 유저에게 보여 준다.
  2. 유저가 수정을 승인하면 해당 이슈를 수정한다.
  3. 수정 완료 후 `/check`를 다시 실행하여 빌드·린트가 통과하는지 확인한다.
  4. 통과하면 Phase 2로 진행한다.
